...
_RESERVED_TARGET_KEYS = frozenset({"area_id", "device_id", "entity_id", "target"})

URL_ENV = "HAP_HOME_ASSISTANT_URL"
TOKEN_ENV = "HAP_HOME_ASSISTANT_TOKEN"
LEGACY_URL_ENV = "WILFRED_HOME_ASSISTANT_URL"
LEGACY_TOKEN_ENV = "WILFRED_HOME_ASSISTANT_TOKEN"
...
def _environment_value(
    ...
    raise HomeAssistantConfigurationError(
        f"Missing environment variable: {canonical}."
    )
...
def reject_target_override(data: Mapping[str, Any]) -> None:
    forbidden = sorted(_RESERVED_TARGET_KEYS.intersection(data))
    if forbidden:
        raise HomeAssistantConfigurationError(
