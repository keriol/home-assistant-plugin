...
def _load_mapping(
    ...
    dict[str, str],
    dict[str, HomeAssistantAction],
]:
    if not path.is_file():
        raise HomeAssistantConfigurationError(
            f"Home Assistant configuration file "
    ...
    try:
        ...
        raise HomeAssistantConfigurationError(
            f"Cannot read Home Assistant "
            f"configuration file: {exc}"
        ) from exc

    unknown = sorted(
        set(document) - _ALLOWED_TOP_LEVEL
    )

    if unknown:
        raise HomeAssistantConfigurationError(
            "Unknown Home Assistant configuration "
            "sections: "
            + ", ".join(unknown)
        )

    raw_targets = _require_table(
        document,
        "targets",
    )

    raw_actions = _require_table(
        document,
        "actions",
    )
    ...
    for name, value in raw_targets.items():
        if not isinstance(value, str):
            raise HomeAssistantConfigurationError(
                f"Target {name!r} must map "
                "to a string entity_id."
            )
    ...
    for name, value in raw_actions.items():
        ...
        unknown_action_keys = sorted(
            set(value) - _ALLOWED_ACTION_KEYS
        )
        ...
        if not isinstance(domain, str):
            raise HomeAssistantConfigurationError(
                f"Action {name!r}.domain "
                "must be a string."
            )

        if not isinstance(service, str):
            raise HomeAssistantConfigurationError(
                f"Action {name!r}.service "
                "must be a string."
            )

        if not isinstance(data, dict):
            raise HomeAssistantConfigurationError(
                f"Action {name!r}.data "
                "must be a TOML table."
            )
